...
/**
 * Encapsulate a command line parameter whose value will be the name of a file or directory. Attributes, such as whether
 * the value is to be a file or directory, whether it must be readable, etc, may be specified and will be validated.
 * <p>
 * Usage:
 * 
 * <pre>
 ...
 * @version jcmdline Rel. @VERSION@ $Id: FileParam.java,v 1.3 2009/08/06 14:31:35 lglawrence Exp $
 * @see CmdLineParser
 */
public class FileParam extends AbstractParameter<File> {
    ...
    /**
     * the default label that will represent option values for this Parameter where {@link #IS_DIR} is set. The
     ...
     * @see "<i>FileParam.defaultDirOptionLabel</i> in 'strings' properties file"
     */
    public static final String DEFAULT_DIR_OPTION_LABEL = Strings.get("FileParam.defaultDirOptionLabel");

    /**
     * the default label that will represent option values for this Parameter where {@link #IS_DIR} is <b>not</b> set.
     ...
     * 
     * @see AbstractParameter#setOptionLabel(String) setOptionLabel()
     * @see "<i>FileParam.defaultFileOptionLabel</i> in 'strings' properties file"
     */
    public static final String DEFAULT_FILE_OPTION_LABEL = Strings.get("FileParam.defaultFileOptionLabel");

    /**
     * indicates that a file or directory specified as a value for this FileParam must <b>not</b> exist
     * 
     ...
     *            a description of the parameter, suitable for display in a usage statement
     * @param optional
     *            {@link Parameter#OPTIONAL OPTIONAL} if optional, {@link Parameter#REQUIRED REQUIRED} if required
     * @throws IllegalArgumentException
     ...
     *            a description of the parameter, suitable for display in a usage statement
     * @param attributes
     *            the attributes that must apply to a file or directory specified as a value to this FileParam
     * @throws IllegalArgumentException
     ...
     *            {@link Parameter#OPTIONAL OPTIONAL} if optional, {@link Parameter#REQUIRED REQUIRED} if required
     * @param multiValued
     *            {@link Parameter#MULTI_VALUED MULTI_VALUED} if the parameter can accept multiple values,
     *            {@link Parameter#SINGLE_VALUED SINGLE_VALUED} if the parameter can contain only a single value
     * @throws IllegalArgumentException
     *             if any parameter is invalid.
     * @see AbstractParameter#setTag(String) setTag()
     * @see AbstractParameter#setDesc(String) setDesc()
     ...
     * <P>
     * If the <code>IS_DIR</code> attribute is specified, the option label for this FileParam will be set to
     * {@link #DEFAULT_DIR_OPTION_LABEL}, else it will be {@link #DEFAULT_FILE_OPTION_LABEL}.
     * 
     * @param tag
     *            a unique identifier for this parameter
     * @param desc
     ...
     *            {@link Parameter#SINGLE_VALUED SINGLE_VALUED} if the parameter can contain only a single value
     * @param hidden
     *            {@link Parameter#HIDDEN HIDDEN} if parameter is not to be listed in the usage, {@link Parameter#PUBLIC
     *            PUBLIC} otherwise.
     * @throws IllegalArgumentException
     *             if any parameter is invalid.
     * @see AbstractParameter#setTag(String) setTag()
     ...
     * 
     * @param attr
     *            one of {@link #NO_ATTRIBUTES}, {@link #EXISTS}, {@link #DOESNT_EXIST}, {@link #IS_DIR},
     *            {@link #IS_FILE}, {@link #IS_READABLE}, or {@link #IS_WRITEABLE}
     * @return <code>true</code> if the attribute is set, <code>false</code> if the attribute is not set or
     *         <code>attr</code> is not a valid attribute
     */
    public boolean attrSpecified(int attr) {
        if (!(attr == EXISTS || attr == NO_ATTRIBUTES || attr == DOESNT_EXIST || attr == IS_DIR || attr == IS_FILE
                || attr == IS_READABLE || attr == IS_WRITEABLE)) {
            return false;
        }
     ...
     */
    @Override
    public File convertValue(String strVal) throws CmdLineException {
        File retval = null;
        try {
            retval = new File(strVal);
        } catch (Exception e) {
            // the following does not return - it throws a CmdLineException
            throwIllegalValueException(strVal);
        }
        return retval;
    }
    ...
    /**
     * Sets the value of attributes. Multiple attributes may be specified by ANDing them together. If multiple
     * attributes are specified, all conditions must be met for a parameter value to be considered valid. For example:
     * 
     * <pre>
     ...
     * <P>
     * Specify <code>NO_ATTRIBUTES</code> if none of the other attributes is required.
     * 
     * @param attributes
     *            a combination of {@link #NO_ATTRIBUTES}, {@link #EXISTS}, {@link #DOESNT_EXIST}, {@link #IS_DIR},
     *            {@link #IS_FILE}, {@link #IS_READABLE}, and {@link #IS_WRITEABLE}
     * @throws IllegalArgumentException
     *             if the attributes value is invalid.
     * @see #getAttributes()
     */
    public void setAttributes(int attributes) {
        if ((attributes ^ 0xffff) >= ((IS_WRITEABLE ^ 0xffff) * 2)) {
            throw new IllegalArgumentException(
                    Strings.get("FileParam.invalidAttributes", new Object[] { new Integer(attributes) }));
        }
        this.attributes = attributes;
    }

    /**
     * Validates a prospective value for the FileParam - called by add/setValue(s)(). All of the attributes are
     * validated and the a CmdLineException is thrown if any are not satisfied.
     * 
     * @param f
     *            the value to validate
     * @throws CmdLineException
     *             if <code>f</code> is not valid.
     * @see #setAttributes(int) setAttributes()
     */
    public void validateValue(File f) throws CmdLineException {
        super.validateValue(f);
        String filename = f.getAbsolutePath();
        if (attrSpecified(IS_DIR) && !f.isDirectory()) {
            throwIllegalValueException(filename);
        }
    ...
    /**
     * Throws a nicely formatted error message when an invalid value is attempted to be validated.
     * 
     * @param val
    ...
    private void throwIllegalValueException(String val) throws CmdLineException {
        ...
        String s2;
        if (attrSpecified(DOESNT_EXIST)) {
            s2 = Strings.get("FileParam.a_not_existing");
        } else if (attrSpecified(EXISTS) || attrSpecified(IS_DIR) || attrSpecified(IS_FILE)
                || attrSpecified(IS_READABLE) || attrSpecified(IS_WRITEABLE)) {
