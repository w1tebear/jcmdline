...
/**
 * An abstract class implementing the Decorator design pattern for decoration of
 * CmdLineHandler subclasses. This class implements all methods of the
 * CmdLineHandler interface by delegating them to a contained instance of
 * CmdLineHandler. The intended use for this class is specification and
 * implementation of a command option that can be reused, and can be used in
 * series with others of the same ilk. More information on CmdLineHandlers and
 * decorator classes can be found in the <a href="doc-files/userguide.html#clh">
 ...
 * <ul>
 * <li>All constructors should accept a <code>CmdLineHandler</code> argument or
 * create a <code>CmdLineHandler</code> to which most method calls will be
 * delegated.
 * <li>They should include a call to this class's constructor, passing their
 * contained <code>CmdLineHandler</code> along as an argument (
 * <code>super(handler)</code>) as the first line of the constructor.
 * <li>In the constructor of the subclass, call setCustomOptions, specifying the
 * options supported by the subclass. This ensures that the options are added to
 * the contained handler, and that they will be restored should setOptions() be
 * called on the contained handler.
 * </ul>
 * <li>Implement the processParsedOptions() method. This method will be called
 * after the command line has been parsed by the contained handler. It is in
 * this method that a subclass should do its option processing.
 * </ul>
 * An example of a subclass follows (note that, but for the lack of additional
 * constructors, this is basically how the {@link VersionCmdLineHandler} is
 * implemented).
 * 
 * <pre>
 ...
 * <p>
 * CmdLineHandler decorator classes are particularly useful for options that
 * perform a task and call System.exit(), such as -help or -version options.
 * Options such as these should have the <code>
 * ignoreRequired</code> attribute set to
 * <code>true</code>. If not, and the command has required parameters, the
 * option will never be accepted unless the required parameters are also
 * specified.
 * <p>
 ...
 * processParsedOptions()</code>
 * method:
 * <ul>
 * <li>This method may never be reached. If the <code>dieOnParseError</code>
 * option is set <code>true</code> for the contained CmdLineHandler (which it is
 * by default), a parse error will cause the program to exit before calling any
 * <code>processParsedOptions()</code> methods.
 * <li>If this method post-processes its option value and finds it to be in
 * error, it should either call {@link #exitUsageError(String) exitUsageError()}
 * or return false, as appropriate based on the value returned by
 * {@link #getDieOnParseError()}. For example:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * <li>If this method does not call <code>System.exit()</code> or return its own
 * error, it should always return the parse error that was passed to it.
 * </ul>
 * 
 * @author Lynne Lawrence
 * @version jcmdline Rel. @VERSION@ $Id: AbstractHandlerDecorator.java,v 1.2
...
public abstract class AbstractHandlerDecorator implements CmdLineHandler {
	 ...
	 * 
	 * @param handler
	 *            the CmdLineHandler to which most methods will be delegated
	 */
	 ...
	 * prefaced with 'ERROR: '. This method does not return - it exits the
	 ...
	 * parse error (after displaying the usage and an error message). This flag
	 ...
	 * This is where a subclass performs processing specific to its custom
	 * options.
	 * 
	 * @param parseStatus
	 *            the results of the parse() call. Note that if
	 *            <code>dieOnParseError</code> is set, or some other
	 *            AbstractHandlerDecorator exits first, this method may never be
	 *            called.
	 * @return <code>true</code> if there were no problems concerning the custom
	 *         options, else <code>false</code>. The return value from this
	 *         method will be returned from {@link #parse(String[]) parse()}.
	 */
