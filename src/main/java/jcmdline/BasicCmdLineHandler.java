 ...
 * <P>
 * Objects of this class use a {@link CmdLineParser} to do the actual parsing of
 * the command line. By default, this class uses a {@link PosixCmdLineParser}.
 * <p>
 * The following semantics are used throughout the documentation for this class.
 * A command line <i>parameter</i> refers to either a command line
 * <i>option</i>, or a command line <i>argument</i>.
 * <P>
 * A command line <i>option</i> is identified by a <i>tag</i> and may or not
 * have an associated value. For instance, in the following posix-type option,
 * "outfile" is the <i>tag</i> and "/tmp/myoutfile" is the <i>value</i>:
 * 
 * <pre>
 * --outfile / tmp / myoutfile
 * </pre>
 * <P>
 * Command line <i>arguments</i> are what are left on the command line after all
 * options have been processed. For example, again using a posix style command
 * line, "filename1" and "filename2" would be the <i>arguments</i>:
 * 
 * <pre>
 *  -e 's/red/blue/' filename1 filename2
 * </pre>
 * <P>
 * Parameters may be designated as <i>hidden</i>. <i>Hidden parameters</i> are
 * those that can be specified, but whose documentation is not displayed to the
 * user when a normal usage is printed.
 * <P>
 ...
 * conjunction with one or more of the {@link AbstractHandlerDecorator} classes
 * that provide some useful options.
 * <P>
 * <b>Post Processing Command Line Parameters</b>
 * <P>
 * It may be the case that none of the supplied Parameter types fully
 * accomodates a program's needs. For instance, a program may require a filename
 * option that is an html filename, ending with '.html'. In this case, the
 * programmer has the options of creating their own Parameter subclass, or
 * post-processing the returned FileParam parameter and generating their own
 * error message. The {@link #exitUsageError(String) exitUsageError()} method is
 * provided so that programs that post-process parameters can take the same exit
 * as would be taken for "normal" parameter processing failures. For instance,
 * in the case just described, the following code could be used to exit the
 * program if the specified file did not end with '.html' (<code>myfile</code>
 * being a FileParam object, and <code>cl</code> being the BasicCmdLineHandler
 * object):
 * 
 * <pre>
 * cl.parse(args);
 ...
 * @version jcmdline Rel. @VERSION@ $Id: BasicCmdLineHandler.java,v 1.4
...
public class BasicCmdLineHandler implements CmdLineHandler {
	...
	/**
	 * the arguments (after the options have been processed) associated with the
	 * command
	 */
	private ArrayList<Parameter<?>> args = new ArrayList<Parameter<?>>();
	 ...
	 */
	private HashMap<String, Parameter<?>> options = new HashMap<String, Parameter<?>>();
	 ...
	 * 
	 * @param cmdName
	 *            the name of the command creating this BasicCmdLineHandler
	 * @param cmdDesc
	 *            a short description of the command's purpose
	 * @param options
	...
	public BasicCmdLineHandler(String cmdName, String cmdDesc,
		...
		this(cmdName, cmdDesc, (Parameter[]) options.toArray(new Parameter[0]),
	...
	/**
	 * Adds a command line arguement. Command line arguments must be added in
	 * the order that they will be specified on the command line.
	 * 
	 * @param arg
	 ...
	 *             if:
	 *             <ul>
	 *             <li><code>arg</code> is null <li>the previously added
	 *             argument was multi-valued (only the last argument can be
	 *             multi-valued) <li><code>arg</code> is a required argument but
	 *             the previous argument was optional
	 *             </ul>
	 */
	public void addArg(Parameter<?> arg) {
		if (arg == null) {
			throw new IllegalArgumentException(Strings
					.get("BasicCmdLineHandler.nullArgNotAllowed"));
		}
		if (args.size() > 0) {
	 ...
	 *             if <code>opt</code> is null.
	 * @throws IllegalArgumentException
	 *             if an option with the same tag has already been added.
	 */
	public void addOption(Parameter<?> opt) {
		...
		if (options.containsKey(opt.getTag())) {
			throw new IllegalArgumentException(Strings.get(
					"BasicCmdLineHandler.duplicateOption", new Object[] { opt
							.getTag() }));
		}
	 ...
	 * prefaced with 'ERROR: '. This method never returns - it exits the program
	...
	public String getUsage(boolean hidden) {
		return parser.getUsageFormatter().formatUsage(cmdName, cmdDesc,
				options, args, hidden);
	}
	 ...
	 * <ul>
	 * <li>the CmdLineParser is unable to parse the command line parameters into
	 * the required options and arguments. <li>a required Parameter has not been
	 * set by the parser.
	 * </ul>
	 * 
	 * @param clargs
	 *            command line arguments passed to the main() method of
	 *            BasicCmdLineHandler's creating class.
	 ...
	 *         {@link #getParseError()}.
	 *         <P>
	 *         If <code>dieOnParseError</code> is set to <code>true</code> and
	 *         the method fails, the program will exit with exit code 1 after
	 *         printing the usage to stderr.
	 */
	...
	/**
	 * Indicates whether the check for required Parameters can be skipped due to
	 * a Parameter with flag <code>ignoreRequired = true</code> having been set.
	 * 
	 * @return true if the check for required parameters may be skipped
	 */
	private boolean canSkipRequiredCheck() {
		for (Parameter<?> p : options.values()) {
	 ...
	 *             if a required option or argument is not set.
	 */
	 ...
	 * @return this method never returns - the program is terminated
	 */
